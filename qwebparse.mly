...
%token FUNCTION END OUTPUT IF OTHERWISE FOR REPEAT INT BOOL FLOAT LIST VOID STRING
%token <int> LITERAL
%token <bool> BLIT
...
stmt:
    expr SEMI                               { Expr $1               }
  | OUTPUT expr_opt SEMI                    { Output $2             }
  | LBRACE stmt_list RBRACE                     { Block(List.rev $2)    }
  | IF LPAREN expr RPAREN stmt %prec NOOTHERWISE { If($3, $5, Block([])) }
  | IF LPAREN expr RPAREN stmt OTHERWISE stmt    { If($3, $5, $7)        }
